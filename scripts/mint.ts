...
import { SneakerERC721Distributor, URIDatabase, SneakerERC721 } from "../typechain";
...
export async function mint() {
  ...
  // if (canmint.isZero()) {
    ...
    const newTotalSupply = totalSupply.toNumber() + 1;
    console.log("New expected total supply:", newTotalSupply);

    if (hardhatArguments.network == "local" && !userConfig.networks?.hardhat?.forking?.enabled) {
      await vrfCoordContract.fulfillRandomWords(newTotalSupply, sneakerContract.address)
    }
  // }
    ...
    if (hardhatArguments.network != "local") {
      getMetaDataToPin(tokenId)
        .then((json) => pinJsonToPinata(json))
          .then((ipfsHash) => sneakerContract.setTokenURI(tokenId, "https://ipfs.io/ipfs/" + ipfsHash));
    }
    else {
      sneakerContract.setTokenURI(newTotalSupply, "https://ipfs.io/ipfs/" + newTotalSupply);
    }
